...
def remove_low_variance_features(X, threshold=0.01):
    st.subheader("🔍 Removing Low Variance Features")
    if X.shape[1] == 0:
        st.warning("⚠️ No features available for variance thresholding.")
        return X
    selector = VarianceThreshold(threshold)
    try:
        X_reduced = selector.fit_transform(X)
        ...
        st.error(f"VarianceThreshold Error: {e}")
        return X

def remove_highly_correlated_features(X, threshold=0.9):
    st.subheader("🧯 Removing Highly Correlated Features")
    if X.shape[1] < 2:
        st.info("Not enough features for correlation removal.")
        return X
    corr_matrix = X.corr().abs()
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
    to_drop = [column for column in upper.columns if any(upper[column] > threshold)]
...
def select_important_features(X, y, method="f_classif", k=10):
    st.subheader("💡 Selecting Top Important Features")
    if X.shape[0] == 0 or X.shape[1] == 0:
        st.warning("⚠️ Feature selection skipped: no samples or features available.")
        return X

    k = min(k, X.shape[1])  # Prevent invalid k
    if method == "f_classif":
...
def scale_features(X):
    st.subheader("⚖️ Final Feature Scaling")
    if X.shape[1] == 0:
        st.warning("⚠️ No features available for scaling.")
        return X
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    st.success("📏 Applied StandardScaler on all features.")
